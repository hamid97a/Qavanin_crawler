...
for row in ruleList:
    ...
    if counter % 20 == 0:
        c2.executemany("INSERT INTO Votes VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",detailsList)
        conn2.commit()
        c1.executemany("DELETE FROM Votes WHERE Id=?",deleteList)
        conn1.commit()
        detailsList = []
        deleteList = []
        print(counter)
